...
@Service
public class SearchCorrelationPrdService {
    ...
    public CorrelationResponse normal(DataAnaysisRequest request) {
		...
		List<TrafficVolumePrdResponse> trafficVolumeResponseList = null;
	    	...
	    	try {
	    		...
	    		if(request.getInterval() < 0) {
		    		...
		    		trafficVolumeResponseList = trafficVolumePrdRepository.findCorrelationTrafficUtcMinusQuery(
		    				request.getSensorId(), 
		    		...
		    		trafficVolumeResponseList = trafficVolumePrdRepository.findCorrelationTrafficUtcPlusQuery(
		    				request.getSensorId(), 
		    				request.getStartDate(), 
		    				request.getEndDate(), 
		    				formatter.getFormatPostgreSql(request.getDateOption()), 
...
//	    	util.exportTrafficPrdCsv(trafficVolumeResponseList);
//	    	util.exportEnvironmentCsv(allSensorViewEntityList);
    ...
    private Map<String, List<double[]>> getCorrelationArrayList(List<TrafficVolumePrdResponse> trafficVolumeResponseList, List<AllSensorViewEntity> allSensorViewEntityList) throws ParseException {
...
//    	List<Double> tuktukList = new ArrayList<>();
        ...
        for(TrafficVolumePrdResponse trafficVolumeResponse : trafficVolumeResponseList){
...
//		xAxisArrayList.add(tuktukList.stream().mapToDouble(Double::doubleValue).toArray());
    ...
    public List<Object> auto(DataAnaysisRequest request) {
    	...
    	try {
    		...
    		if(request.getInterval() < 0) {
	    		...
	    		trafficVolumeResponseList = trafficVolumePrdRepository.findTrafficVolumeSumAvgUtcMinusQuery(
	    				request.getSensorId(), 
	    				request.getStartDate(), 
	    				request.getEndDate(), 
	    		...
	    		trafficVolumeResponseList = trafficVolumePrdRepository.findTrafficVolumeSumAvgUtcPlusQuery(
	    				request.getSensorId(), 
	    				request.getStartDate(), 
	    				request.getEndDate(), 
	    				request.getDateOption(), 
    ...
    private TrafficAutoCorrelationResponse trafficAutocorrelation(List<TrafficVolumePrdResponse> dataList, int lag) {
    	...
    	for(TrafficVolumePrdResponse data : dataList) {
